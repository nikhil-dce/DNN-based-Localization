    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
            ...
            for i in xrange(FLAGS.num_gpus):
                with tf.device('/gpu:%d' % i):
                    with tf.name_scope('%s_%d' % (TOWER_NAME, i)) as scope:
                        ...
                        output = model.inference(images, phaseTrain)
            ...
            for i in xrange(FLAGS.num_gpus):
                with tf.device('/gpu:%d' % i):
                    with tf.name_scope('Validation_%s_%d' % (TOWER_NAME, i)) as scope: # This will only change name scopes for operations (Vars still the same)
                        ...
                        # Calculating tower loss
                        output = model.inference(images, phaseValidation)
                        batch_loss_array = model.batch_loss(output, labels) 
        ...
        for step in range(FLAGS.max_steps):
            ...
            if step % FLAGS.save_pred_every == 0:
                loss_value, _ , summary = sess.run([loss, train_op, summary_op], feed_dict=feed_dict)
